...
#
# By default, PeriodicGC polls every 1 second, and if more than 0 bytes have been allocated on the heap, it runs garbage collection. To make PeriodicGC less aggressive (and use less CPU time, at the cost of higher memory usage), you may raise both of these settings, for example: `PeriodicGC::Timer.poll_interval = 5.seconds` and `PeriodicGC::Timer.bytes_since_gc_threshold = 128*1024`.
#
# Idle detection is based on a measurement of how long it takes `Fiber.yield` to return, and can be tuned with `PeriodicGC::Idle.idle_threshold=`. If interactive performance (latency) is not a concern for your application, it is recommended that you disable idle detection with `PeriodicGC::Timer.only_if_idle = false`.
#
# Since idle detection may be inaccurate, there is a `PeriodicGC::Timer.force_gc_period=` which is set to force a collection every 2 minutes by default. You may disable this with `PeriodicGC::Timer.force_gc_period = nil`.
#
